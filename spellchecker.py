...
WORDS = words(open('dictionary_lower.txt').read())
...
def candidates(word):
    ...
    a = list(known([word]) or known(edits1(word)) or known(edits2(word)) or [word])
    if ((len(a) is 1) and (a[0] is word)):
        return []
    else:
        return a
